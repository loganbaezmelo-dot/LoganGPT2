  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (imagenMode) {
        ...
        replyText = `🎨 **Image Generated**\n\n![${userText}](${imageUrl})\n\n*Processing complete. Asset deployed.*`;
                ...
                <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`max-w-[85%] rounded-2xl p-4 text-sm leading-7 shadow-md border ${msg.role === 'user' ? 'bg-slate-800 text-white border-white/5' : 'bg-slate-900/50 text-slate-200 border-white/5'}`}>
                    <ReactMarkdown 
                      ...
                      components={{
                        img: ({node, ...props}) => <img {...props} className="rounded-lg shadow-lg max-w-full h-auto border border-white/10" alt="Generated" />
                      }}
                    >
                      {msg.text}
                    </ReactMarkdown>
