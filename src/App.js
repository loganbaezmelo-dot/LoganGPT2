...
import { 
  ...
  Key, Palette, ExternalLink, User, WifiOff, Image as ImageIcon, Sparkles, CheckCircle, Paintbrush
} from 'lucide-react';
...
const LOCAL_BRAIN = [
  { 
    ...
    response: "I am **LoganGPT**, a React-based AI assistant. I use Google Gemini for text and Pollinations AI for image generation." 
  },
  { 
    triggers: ["tiers", "pricing", "cost", "plans"], 
    response: "I operate on a simple model:\n\n* **Standard Tier:** Text processing via Google Gemini.\n* **Creative Tier:** Image generation powered by Pollinations AI (Included/Free)." 
  },
  { 
    triggers: ["how do you make images", "image engine", "pollinations"], 
    response: "I utilize the **Pollinations AI** public API to generate images on the fly. It's a fast, open-source alternative to proprietary models like DALL-E or Imagen." 
  },
  { 
    triggers: ["hello", "hi"], 
    response: "Yo. Ready to create? 🚀" 
  }
];

// FIXED: Removed the forced table instruction so it speaks normally
const SYSTEM_PROMPT = "You are LoganGPT. Helpful, professional, and precise. Format responses clearly using Markdown.";
    ...
    <div className="flex h-screen w-full items-center justify-center bg-[#0B1120] text-slate-100 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm mt-2">AI Workspace</p>
        </div>
...
export default function App() {
  ...
  const [creativeMode, setCreativeMode] = useState(false); // Renamed from imagenMode
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // --- LOGIC FOR CREATIVE (Pollinations) vs GEMINI ---
      let replyText = "";
      
      if (creativeMode) {
        // 🔥 HONEST IMAGE GENERATION
        await new Promise(r => setTimeout(r, 1500)); // Slight processing delay
  ...
  return (
    <div className="flex h-[100dvh] bg-[#0B1120] text-slate-100 font-sans overflow-hidden"
         style={{ '--accent': creativeMode ? '#ec4899' : theme.color, '--accent-hover': creativeMode ? '#db2777' : theme.hover }}>
           ...
           <div className="flex items-center gap-3 mb-3 px-1">
             ...
             <div className="flex-1 min-w-0">
               ...
               <div className="text-[10px] text-slate-500">
                 {creativeMode ? 'Creative Tier' : 'Standard Tier'}
               </div>
             </div>
           </div>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 relative bg-[#0B1120]">
        ...
        <header className="absolute top-0 w-full bg-[#0B1120]/80 backdrop-blur-md border-b border-white/5 px-4 py-3 flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className={`p-2 rounded-lg transition-all flex items-center gap-2 text-xs font-bold ${creativeMode ? 'bg-pink-600 text-white shadow-lg shadow-pink-500/20' : 'bg-white/5 text-slate-400 hover:text-white hover:bg-white/10'}`}
              title="Toggle Image Generation"
            >
              <ImageIcon className="w-4 h-4" />
              <span className="hidden sm:inline">{creativeMode ? 'ON' : 'OFF'}</span>
            </button>
          ...
          {!activeChatId ? (
            <div className="max-w-2xl mx-auto mt-12 md:mt-20 text-center space-y-8 px-4 animate-fade-in">
              <div className="relative w-20 h-20 mx-auto">
                <div className={`absolute inset-0 bg-gradient-to-tr rounded-2xl blur-xl opacity-50 animate-pulse ${creativeMode ? 'from-pink-500 to-rose-500' : 'from-violet-500 to-indigo-500'}`}></div>
                <div className="relative w-20 h-20 bg-slate-900 rounded-2xl border border-white/10 flex items-center justify-center shadow-2xl" style={{ borderColor: creativeMode ? '#ec4899' : theme.color }}>
                  {creativeMode ? <Paintbrush className="w-10 h-10 text-pink-400" /> : <Monitor className="w-10 h-10" style={{ color: theme.color }} />}
                </div>
              </div>
              
              <div>
                <h2 className="text-3xl font-bold text-white mb-2">{creativeMode ? "Creative Mode Active." : "System Online."}</h2>
                <p className="text-slate-400">
                  {creativeMode ? "Generates images via Pollinations AI." : (apiKey ? "Connected to Cloud Intelligence." : "Running in Offline Mode.")}
                </p>
              </div>
            ...
            <div className="max-w-3xl mx-auto space-y-6">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center text-xs font-bold border border-white/10 shadow-lg ${msg.role === 'user' ? 'bg-slate-800 text-slate-300' : 'text-white'}`} style={msg.role === 'model' ? { backgroundColor: creativeMode && msg.role === 'model' ? '#ec4899' : theme.color } : {}}>
                    {msg.role === 'user' ? <User className="w-4 h-4"/> : (creativeMode && msg.role === 'model' ? <Paintbrush className="w-4 h-4"/> : <Zap className="w-4 h-4" fill="currentColor"/>)}
                  </div>
        ...
        <footer className="fixed bottom-0 w-full lg:w-[calc(100%-18rem)] bg-gradient-to-t from-[#0B1120] via-[#0B1120] to-transparent p-4 z-20">
          <div className={`max-w-3xl mx-auto relative backdrop-blur-xl rounded-2xl flex items-center shadow-2xl border transition-colors ring-1 ring-white/5 ${creativeMode ? 'bg-pink-950/40 border-pink-500/30' : 'bg-[#1e293b]/50 border-white/10'}`}>
            <input 
              type="text" 
              value={input} 
              ...
              placeholder={creativeMode ? "Describe an image to generate..." : "Message LoganGPT..."}
              onKeyDown={(e) => e.key === 'Enter' && handleSend(e)}
            />
            <button onClick={handleSend} disabled={isLoading || !input.trim()} className="p-2.5 mr-2 text-white rounded-xl hover:opacity-90 transition-all disabled:opacity-50" style={{ backgroundColor: creativeMode ? '#ec4899' : theme.color }}>
              {creativeMode ? <Sparkles className="w-4 h-4" /> : <Send className="w-4 h-4" />}
            </button>
          </div>
          <p className="text-center text-[10px] text-slate-600 mt-2">{creativeMode ? "Image generation may incur separate costs." : "AI may produce inaccurate information."}</p>
        </footer>
      </div>
