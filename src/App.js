...
import { 
  ...
  Sparkles, CheckCircle, Paintbrush, Layout, Play, Maximize2
} from 'lucide-react';
...
const SYSTEM_PROMPT_STANDARD = "You are LoganGPT. Helpful, professional, and precise. When providing data, use markdown tables.";
...
export default function App() {
  ...
  // Helper to get current accent color based on mode
        ...
        <header className="absolute top-0 w-full bg-[#0B1120]/80 backdrop-blur-md border-b border-white/5 px-4 py-3 flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className={`p-2 rounded-lg transition-all flex items-center gap-2 text-xs font-bold ${currentMode === 'creative' ? 'bg-pink-600 text-white shadow-lg shadow-pink-500/20' : 'bg-white/5 text-slate-400 hover:text-white hover:bg-white/10'}`}
              title="Creative Mode"
            >
            ...
            <button 
              ...
              className={`p-2 rounded-lg transition-all flex items-center gap-2 text-xs font-bold ${currentMode === 'canvas' ? 'bg-yellow-600 text-white shadow-lg shadow-yellow-500/20' : 'bg-white/5 text-slate-400 hover:text-white hover:bg-white/10'}`}
              title="Canvas Mode"
            >
        ...
        <footer className="fixed bottom-0 w-full lg:w-[calc(100%-18rem)] bg-gradient-to-t from-[#0B1120] via-[#0B1120] to-transparent p-4 z-20">
          <div className={`max-w-3xl mx-auto relative backdrop-blur-xl rounded-2xl flex items-center shadow-2xl border transition-colors ring-1 ring-white/5 ${currentMode === 'creative' ? 'bg-pink-950/40 border-pink-500/30' : currentMode === 'canvas' ? 'bg-yellow-950/40 border-yellow-500/30' : 'bg-[#1e293b]/50 border-white/10'}`}>
            <input 
              ...
              placeholder={currentMode === 'creative' ? "Describe an image..." : currentMode === 'canvas' ? "Describe an app to build..." : "Message LoganGPT..."}
                ...
                <>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center text-[10px] text-slate-500">
                      ...
                      <a href="[https://aistudio.google.com/app/apikey](https://aistudio.google.com/app/apikey)" target="_blank" rel="noreferrer" className="flex items-center gap-1 text-violet-400 hover:text-violet-300">
