...
function Login({ onLogin }) {
  ...
  const handleGoogle = async () => {
    try {
      ...
      setLoading(false);
      // 💀 FIX: Ignore the error if user just closed the popup
      if (err.code === 'auth/popup-closed-by-user') {
        console.log("User closed the popup. No biggie.");
        return;
      }
  ...
  return (
    <div className="flex h-screen w-full items-center justify-center bg-slate-950 text-slate-100 p-4">
      <div className="w-full max-w-md bg-slate-900 border border-white/10 rounded-2xl p-8 shadow-2xl">
        ...
        {error && (
          <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 text-red-400 text-xs rounded-lg animate-pulse">
            {error}
          </div>
        )}
          ...
          {!activeChatId ? (
            <div className="max-w-2xl mx-auto mt-16 text-center space-y-6 px-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-lg mx-auto">
                <button onClick={() => { setInput("What can you do?"); }} className="p-4 rounded-xl border border-white/10 hover:bg-white/5 text-left text-sm text-slate-300">"What can you do?"</button>
                <button onClick={() => { setInput("Debug this code"); }} className="p-4 rounded-xl border border-white/10 hover:bg-white/5 text-left text-sm text-slate-300">"Debug this code"</button>
              </div>
            </div>
          ) : (
