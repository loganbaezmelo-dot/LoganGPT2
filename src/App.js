...
const SYSTEM_PROMPT = "You are LoganGPT. Helpful, professional, and precise. When providing data, prefer using markdown tables.";
...
export default function App() {
  ...
  const [imagenMode, setImagenMode] = useState(false); 
  ...
  const saveSettings = () => {
    ...
    // Cleaned up imagenKey logic since we removed the input
    ...
    <div className="flex h-[100dvh] bg-[#0B1120] text-slate-100 font-sans overflow-hidden"
           ...
           <div className="flex items-center gap-3 mb-3 px-1">
             ...
             <div className="flex-1 min-w-0">
               ...
               <div className="text-[10px] text-slate-500">
                 {imagenMode ? 'Images Tier' : 'Free Tier'}
               </div>
             </div>
           </div>
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 relative bg-[#0B1120]">
        ...
        <header className="absolute top-0 w-full bg-[#0B1120]/80 backdrop-blur-md border-b border-white/5 px-4 py-3 flex items-center justify-between z-10">
          ...
          <div className="flex items-center gap-2">
            {/* --- IMAGEN MODE TOGGLE (Always Visible Now) --- */}
            <button 
              ...
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center text-xs font-bold border border-white/10 shadow-lg ${msg.role === 'user' ? 'bg-slate-800 text-slate-300' : 'text-white'}`} style={msg.role === 'model' ? { backgroundColor: imagenMode && msg.role === 'model' ? '#6366f1' : theme.color } : {}}>
                    {msg.role === 'user' ? <User className="w-4 h-4"/> : (imagenMode && msg.role === 'model' ? <ImageIcon className="w-4 h-4"/> : <Zap className="w-4 h-4" fill="currentColor"/>)}
                  </div>
                  <div className={`max-w-[85%] rounded-2xl p-4 text-sm leading-7 shadow-md border ${msg.role === 'user' ? 'bg-slate-800 text-white border-white/5' : 'bg-slate-900/50 text-slate-200 border-white/5'}`}>
                    <ReactMarkdown 
                      ...
                      remarkPlugins={[ReactMarkdown.remarkGfm]} // Ensure GFM support
                      components={{
            ...
            <div className="p-6 space-y-8 overflow-y-auto">
              ...
              {settingsTab === 'general' ? (
                ...
                <div className="space-y-4">
                  ...
                  {/* Images Tier Card (Included by default now) */}
                  <div className={`p-4 rounded-xl border border-indigo-500/50 bg-indigo-500/10`}>
                    <div className="flex justify-between items-center mb-2">
                      <h3 className="font-bold text-white flex items-center gap-2">
                        <ImageIcon className="w-4 h-4 text-indigo-400" /> Images Tier
                      </h3>
                      <span className="text-[10px] bg-indigo-500 text-white px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
                        <CheckCircle className="w-3 h-3" /> ACTIVE
                      </span>
                    </div>
                    <p className="text-xs text-slate-400 mb-3">Enterprise visual reasoning capabilities enabled.</p>
                    <ul className="text-xs text-slate-400 space-y-1 ml-6 list-disc">
                      <li>High-fidelity Image Generation</li>
                      <li>Visual Context Processing</li>
